...
def get_basic_model(config=None):
    ...
    if not config:
        config = config_class.from_pretrained('FacebookAI/roberta-base')
    else:
        config = config
    config.output_hidden_states = True
    # print(config)
    # tokenizer = tokenizer_class.from_pretrained('microsoft/codebert-base')
    nl_model = model_class.from_pretrained('FacebookAI/roberta-base',
                                           config=config, from_tf=False)
    tokenizer = tokenizer_class.from_pretrained('FacebookAI/roberta-base', do_lower_case=True)
    nl_model.to(device)
...
config_path = Path('tiny_roberta_config.json')
...
print('loading data.................')
files = os.listdir('../data/DistillData/')
...
df = mergedf(corpus_df, '../data/DistillData/', files)
...
with distiller:
        distiller.train(optimizer, mydataloader, num_epochs=num_epochs, scheduler_class=scheduler_class, scheduler_args=scheduler_args,
                        callback=None)

print('robertadistill')
